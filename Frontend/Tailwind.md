Tailwind is a utility first [[CSS]] framework. Instead of creating predefined classes it creates a list of utility classes that can be mixed and matched. 

Currently the most popular CSS  framework

Automatically scans your files and creates a css file containing only the classes you actually use.

| Advantages                                          | Disadvantages                              |
| --------------------------------------------------- | ------------------------------------------ |
| Design freedom, not limited to bootstrap look       | Harsher design curve, you have to know css |
| No CSS weight, the created production file is light | HTML can be hard to read                   |
| Easy to maintain                                    | No ready components, you have to make them |
|                                                     |                                            |
